...
class TestVenueDetailPageView:
    ...
    def test_returns_areas_sorted_by_order_then_name(
        ...
        area_c = Area.objects.create(
            event=event, venue=venue, name="Charlie Wing", slug="charlie", order=2
        )
        area_a = Area.objects.create(
            event=event, venue=venue, name="Alpha Wing", slug="alpha", order=1
        )
        area_b = Area.objects.create(
            event=event, venue=venue, name="Beta Wing", slug="beta", order=1
        )
    ...
    def test_only_shows_areas_for_current_venue(
        ...
        area1 = Area.objects.create(
            event=event, venue=venue1, name="Area 1", slug="area-1"
        )
        # Create area for venue2
        Area.objects.create(event=event, venue=venue2, name="Area 2", slug="area-2")
