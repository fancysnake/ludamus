...
def _format_date_range(start: datetime, end: datetime) -> str:
    ...
    if start_date.year == end_date.year:
        if start_date.month == end_date.month:
            month_name = date_format(start, format="F", use_l10n=True)
            ...
            return f"{month_name} {start_day}–{end_day}, {year}, {start_time} – {end_time}"
        else:
            ...
            start_time = time_format(start, format="TIME_FORMAT", use_l10n=True)
            end_time = time_format(end, format="TIME_FORMAT", use_l10n=True)
            return f"{start_month} {start_day} – {end_month} {end_day}, {year}, {start_time} – {end_time}"
    else:
...
@register.filter
def format_datetime_range(start: Union[datetime, Any], end: Union[datetime, Any]) -> str:
