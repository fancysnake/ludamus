...
IMAGES = {
    ...
    "event_tabletop": (
        "https://images.unsplash.com/photo-1610484826967-09c5720778c7?q=80&w=2070&auto=format&fit=crop"
    ),
    "event_halloween": (
    ...
    "session_horror": (
        "https://images.unsplash.com/photo-1518331483807-f6adc0e1ad80?q=80&w=2069&auto=format&fit=crop"
    ),
    "session_scifi_1": (
        "https://images.unsplash.com/photo-1535030456952-3791b811d806?q=80&w=2069&auto=format&fit=crop"
    ),
    "session_scifi_2": (
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: Any) -> None:  # noqa: PLR6301
        parser.add_argument(
            "--no-flush",
            action="store_true",
    ...
    def _set_image(self, instance: models.Model, url: str) -> None:
        ...
        try:
            ...
            if response.status_code == HTTPStatus.OK:
                filename = f"{url.rsplit('/', 1)[-1].split('?')[0]}.jpg"
                instance.image.save(filename, ContentFile(response.content), save=True)
                self.stdout.write(f"Set image for {instance}")
        except requests.RequestException as exc:
            self.stderr.write(f"Failed to fetch image {url}: {exc}")

    def _create_upcoming_event(self, sphere: Sphere, tags: Sequence[Tag]) -> None:
        now = timezone.now()
    ...
    def _create_live_event(self, sphere: Sphere, tags: Sequence[Tag]) -> None:
        now = timezone.now()
        event, created = Event.objects.get_or_create(
            sphere=sphere,
            slug="live-gaming",
    ...
    def _create_session(  # noqa: PLR0913, PLR0917
        ...
        space, _ = Space.objects.get_or_create(
            event=event, name=location, defaults={"slug": f"{event.slug}-{slug}-space"}
        )
