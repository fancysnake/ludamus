...
class VenuesPageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(self, _request: PanelRequest, slug: str) -> HttpResponse:
        ...
        context["venues"] = self.request.uow.venues.list_by_event(current_event.pk)
...
class VenuesStructurePageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def _build_venue_structure(self, event_pk: int) -> list[dict[str, Any]]:
        ...
        """
        venues = self.request.uow.venues.list_by_event(event_pk)
        structure = []

        for venue in venues:
            areas = self.request.uow.areas.list_by_venue(venue.pk)
            venue_data: dict[str, Any] = {"venue": venue, "areas": []}
            for area in areas:
                spaces = self.request.uow.spaces.list_by_area(area.pk)
                venue_data["areas"].append({"area": area, "spaces": list(spaces)})
...
class VenueCreatePageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(self, _request: PanelRequest, slug: str) -> HttpResponse:
        ...
        self.request.uow.venues.create(current_event.pk, name, address)
...
class VenueEditPageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        try:
            venue = self.request.uow.venues.read_by_slug(current_event.pk, venue_slug)
        except NotFoundError:
    ...
    def post(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        name = form.cleaned_data["name"]
        address = form.cleaned_data.get("address") or ""
        self.request.uow.venues.update(venue.pk, name, address)
...
class VenueReorderActionView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(self, _request: PanelRequest, slug: str) -> HttpResponse:
        ...
        self.request.uow.venues.reorder(current_event.pk, venue_ids)
...
class VenueDuplicatePageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        context["active_nav"] = "venues"
        context["venue"] = venue
        context["form"] = VenueDuplicateForm(initial={"name": f"{venue.name} (Copy)"})
    ...
    def post(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        new_venue = self.request.uow.venues.duplicate(
...
class VenueCopyPageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        context["active_nav"] = "venues"
        context["venue"] = venue
        context["form"] = VenueCopyForm(events=event_choices)
    ...
    def post(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        form = VenueCopyForm(self.request.POST, events=event_choices)
        if not form.is_valid():
            context["active_nav"] = "venues"
            context["venue"] = venue
            context["form"] = form
        ...
        self.request.uow.venues.copy_to_event(venue.pk, target_event_id)
        messages.success(
            self.request,
...
class AreaCreatePageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        self.request.uow.areas.create(current_event.pk, venue.pk, name, description)
...
class AreaEditPageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(
        ...
        try:
            area = self.request.uow.areas.read_by_slug(venue.pk, area_slug)
        except NotFoundError:
    ...
    def post(
        ...
        name = form.cleaned_data["name"]
        description = form.cleaned_data.get("description") or ""
        self.request.uow.areas.update(area.pk, name, description)
...
class AreaReorderActionView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(self, _request: PanelRequest, slug: str, venue_slug: str) -> HttpResponse:
        ...
        self.request.uow.areas.reorder(venue.pk, area_ids)
...
class SpaceCreatePageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(
        ...
        self.request.uow.spaces.create(current_event.pk, area.pk, name, capacity)
...
class SpaceEditPageView(PanelAccessMixin, EventContextMixin, View):
    ...
    def get(
        ...
        try:
            space = self.request.uow.spaces.read_by_slug(area.pk, space_slug)
        except NotFoundError:
    ...
    def post(
        ...
        name = form.cleaned_data["name"]
        capacity = form.cleaned_data.get("capacity")
        self.request.uow.spaces.update(space.pk, name, capacity)
...
class SpaceReorderActionView(PanelAccessMixin, EventContextMixin, View):
    ...
    def post(
        ...
        self.request.uow.spaces.reorder(area.pk, space_ids)
